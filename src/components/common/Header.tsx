...
// 2. Atualize a interface para aceitar 'name' opcional
export interface UserProfile {
    avatarSource?: ImageSourcePropType;
    initials?: string; // Agora opcional
    name?: string;     // Novo campo para fallback
}
...
export const Header = ({ userProfile, onPressProfile, notificationCount = 0, onPressNotifications }: HeaderProps) => {
    ...
    // Tenta usar initials -> Se não tiver, calcula do nome -> Se não tiver nome, mostra '?'
    const textToDisplay = userProfile?.initials 
        ? userProfile.initials 
        : getInitialsFromName(userProfile?.name || '') || '?';
                ...
                <TouchableOpacity onPress={onPressProfile} style={styles.actionButton} disabled={!userProfile}>
                    {userProfile?.avatarSource ? (
                        ...
                        <View style={[styles.avatar, styles.initialsContainer]}>
                            ...
                            <Text style={styles.initialsText}>{textToDisplay}</Text>
                        </View>
                    )}            
                </TouchableOpacity>
            </View>
