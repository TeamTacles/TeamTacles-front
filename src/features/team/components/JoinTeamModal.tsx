...
export const JoinTeamModal: React.FC<JoinTeamModalProps> = ({ visible, onClose, onSuccess }) => {
  ...
  const handleJoin = async () => {
    if (!token.trim()) {
      setInfoPopup({ visible: true, title: 'Atenção', message: 'Por favor, cole o código da equipe.', type: 'error' });
      return;
    }

    setLoading(true);
    try {
      await teamService.joinTeamWithLink(token.trim());
      
      // Sucesso!
      setInfoPopup({ visible: true, title: 'Sucesso!', message: 'Você entrou na equipe.', type: 'success' });
      ...
      const errorMessage = error.response?.data?.message || 'Código inválido ou você já está nesta equipe.';
    ...
    <Modal animationType="slide" transparent={true} visible={visible} onRequestClose={handleClose}>
      <View style={styles.centeredView}>
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.modalView}>
          ...
          <Text style={styles.modalTitle}>Entrar em Equipe</Text>
